...
import { useGetTblPropTypesQuery, useDeleteTblPropTypeMutation } from "../../../../redux/api/tblPropTypeApi";
...
const BankAccPage = () => {
    ...
    const [editingTblPropType, setEditingTblPropType] = useState<any>(null);
    const [sorting, setSorting] = useState<SortingState>([]);
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
    const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({});
    const [deleteTblPropType] = useDeleteTblPropTypeMutation();
    const { data, isLoading, error, refetch } = useGetTblPropTypesQuery({
        perPage,
        page: currentPage,
        search,
    });
    const tblPropTypes: any = data?.data || [];
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: "PropTypeID",
                header: () => <span style={{ fontSize: "16px", fontWeight: "bold" }}>PropertyTypeID</span>,

            },
            {
                accessorKey: "PropertyType",
                header: () => <span style={{ fontSize: "16px", fontWeight: "bold" }}>Property Type</span>,
                ...
                cell: ({ row }: any) => (
                    <div className="d-flex gap-2">
                        <Button
                            ...
                            size="sm"
                            onClick={() => setEditingTblPropType(row.original)}
                        >
                            Edit
                        </Button>
                        <Button
    ...
    const handleDelete = async (tbl_PropType: any) => {
        ...
        const result = await Swal.fire({
            title: "Are you sure?",
            text: `Do you want to delete "${tbl_PropType.PropertyType}"?`,
            icon: "warning",
        ...
        if (result.isConfirmed) {
            try {
                await deleteTblPropType(tbl_PropType.PropTypeID).unwrap();
                Swal.fire({
                    icon: "success",
                    title: "Deleted",
        ...
        <Container fluid className="p-4">
            ...
            <nav aria-label="breadcrumb" className="mb-3">
                <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item active" aria-current="page">Tbl Prop Type List</li>
                </ol>
            </nav>
            ...
            <Card className="p-3 shadow-sm">
                <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                    ...
                    <Button variant="primary" onClick={() => setShowCreateModal(true)}>
                        <Plus size={23} strokeWidth={2} className="" />   Add Tbl Prop Type
                    </Button>
                </div>
                ...
                {editingTblPropType && (
                    <Edit
                        show={!!editingTblPropType}
                        handleClose={() => setEditingTblPropType(null)}
                        TblPropType={editingTblPropType}
                        refetch={refetch}
                    />
                )}
